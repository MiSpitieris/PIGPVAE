...
class GPVAE(nn.Module):
    def __init__(
        ...
        self.q_net = q_net(input_size, hidden_layers_encoder, latent_dim, activation_encoder, logvar_range=(-3,10))
    ...
    def generate_and_plot(self, mu_s, t_uni, num_samples, ylim, df, plot=True, seed=None):
        ...
        Args:
            ...
            df: Original data DataFrame with 'interval', 'time_within_interval', and 'B.RTD1' columns.
        ...
        if plot:
            ...
            for dec in generated_data:
                axs[1].plot(t_uni*240, dec.cpu().numpy())
            ...
            for i in df['interval'].unique():
                id = df['interval'] == i
                axs[0].plot(df[id]['time_within_interval'].to_numpy(), df[id]['B.RTD1'].to_numpy(),
