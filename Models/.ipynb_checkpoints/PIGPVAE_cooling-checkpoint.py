...
class PIGPVAE(nn.Module):
    def __init__(
        ...
        device="cpu",
        initial_alpha=0.5  # Added parameter for initial alpha value
    ):
    ...
    def elbo(self, X, beta_phy, beta_delta):
        ...
        reg_loss = -regularization_loss(x_hat, x_hat_phy)# -regularization_loss(x_hat_delta, torch.zeros_like(x_hat_delta))
    ...
    def fit(self, train_loader, val_loader, opt, num_epochs=1500, beta_phy=0.0001, beta_delta=0.0001, seed=0, 
            plot=False, df_c=None, ns=30, T0=None, Ts=None, t_uni=None, axs_ylim=(0, 35)):
        ...
        for epoch in tqdm(epochs, desc="Training Progress"):
            ...
            for x_batch in train_loader:
                ...
                d_train = self.train_step(x_batch.squeeze(), opt, beta_phy, beta_delta)
            ...
            with torch.no_grad():
                for x_batch_val in val_loader:
                    ...
                    d_val = self.val_step(x_batch_val.squeeze(), beta_phy, beta_phy, beta_delta)
            ...
            if (epoch + 1) % progress_interval == 0 or epoch == num_epochs - 1:
                ...
                if plot:
                    ...
                    # self.generate_and_plot(mu_s, mean_q_mu_phy, mean_q_var_phy, df_c, num_unique_intervals, ns, T0=None, Ts=None, t_uni=None, axs_ylim=(0, 35), plot=True)
                    self.generate_and_plot(mu_s, mean_q_mu_phy, mean_q_var_phy, df_c, num_unique_intervals, ns, T0=T0, Ts=Ts, t_uni=t_uni, axs_ylim=axs_ylim, plot=True)
    ...
    def generate_and_plot(self, mu_s, mean_q_mu_phy, mean_q_var_phy, df_c, num_unique_intervals, ns, T0=None, Ts=None, t_uni=None, axs_ylim=(0, 35), plot=True, seed=None):
        """
        ...
        Args:
            df_c: DataFrame containing the original data with intervals and temperature readings.
            num_unique_intervals: Number of unique intervals in df_c.
        ...
        interval_to_color = {interval: colors[idx] for idx, interval in enumerate(df_c['interval'].unique())}

        # Plot each unique interval from the original data (df_c) on axs[0]
        if plot:
            for interval in df_c['interval'].unique():
                id_mask = df_c['interval'] == interval
                axs[0].plot(
                    df_c[id_mask]['time_within_interval'].to_numpy(),
                    df_c[id_mask]['B.RTD1'].to_numpy(),
        ...
        for i in range(ns):
            with torch.no_grad():
                # Sample z_delta and z_phy using the model's methods
    ...
    def generate_and_plot2(self, mu_s, mean_q_mu_phy, mean_q_var_phy, df_c, num_unique_intervals, ns, T0=None, Ts=None, t_uni=None, axs_ylim=(0, 35), plot=True, seed=None):
        """
        Method to generate and plot both the generated and original data from the model.
        This method integrates the plot_generated_vs_original_data function into the class.
