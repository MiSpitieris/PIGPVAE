...
class PIGPVAE(nn.Module):
    ...
    def train_step(self, X, opt, beta_phy, beta_delta):
        opt.zero_grad()
        elb, KL_delta, KL_phy, mse, ce = self.elbo(X, beta_phy, beta_delta)
        mse = -mse
        loss = -elb
        loss.backward()
        opt.step()
    ...
    def fit(self, train_loader, val_loader, opt, num_epochs=1500, beta_phy=0.0001, beta_delta=0.0001, seed=0, 
        ...
        for epoch in tqdm(epochs, desc="Training Progress"):
            batch_losses, batch_KL_phy, batch_KL_delta, batch_mse, batch_ce = [], [], [], [], []
            batch_lengthscales, batch_outputscales = [], []
            ...
            # Validation loop
            self.eval()
            val_batch_losses, val_batch_KL_phy, val_batch_KL_delta, val_batch_mse, val_batch_ce = [], [], [], [], []
            val_batch_lengthscales, val_batch_outputscales = [], []
